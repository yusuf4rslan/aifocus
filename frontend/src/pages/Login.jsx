...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(data);
      toast.success('Tekrar hoş geldin, Yusuf!');
      navigate('/');
    } catch (error) {
      toast.error(error.response?.data?.message || 'Giriş yapılamadı. Bilgileri kontrol et.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-3xl border border-gray-100 shadow-xl shadow-teal-500/5">
        <div className="text-center">
          ...
          <h2 className="text-4xl font-black text-slate-900 tracking-tight mb-2">
            HESABINA <span className="text-teal-600 underline decoration-teal-200 decoration-4 underline-offset-4">DÖN</span>
          </h2>
          <p className="text-slate-500 font-medium">Yapay Zeka Gündemi seni bekliyor.</p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={loading}
            className="group relative w-full flex justify-center py-4 px-4 border border-transparent text-sm font-black rounded-2xl text-white bg-slate-900 hover:bg-teal-600 focus:outline-none transition-all duration-300 transform hover:-translate-y-1 active:scale-95 disabled:opacity-70 disabled:hover:translate-y-0"
          >
            {loading ? 'KONTROL EDİLİYOR...' : 'GİRİŞ YAP VE BAŞLA'}
          </button>
        </form>
